...
#include <string.h>
#include <math.h>
//#include <cfile.h>    /* may want to remove this line */
...
void susan_principle( 
					 uchar *in,int   *r,uchar *bp,
...
void susan_smoothing(
					 int   three_by_three,uchar *in,float dt,int x_size,int y_size,
					 uchar *bp
					 )
{
	/* {{{ vars */
			   ...
			   int x_size,int y_size,int drawing_mode)
{
	...
	{
		if (*midp<8) 
			*(in + (midp - mid)) = 0;//edge pixels
		else
			*(in + (midp - mid)) = 255;//non edge candidate pixels.
		midp++;
	}
}
...
void susan_edges( 
				 uchar *in,int   *r,uchar *mid,uchar *bp,
				 int max_no,int x_size,int y_size)
{
	float z;
...
//parameters required: in, r, bp, bt, max-no-edges, x-size, y-size
void susanedge(unsigned char *in,int x_size, int y_size,int bt=20, bool three_by_three=false)
{
	
	uchar proxy[516]={0};
	uchar  *bp=proxy, *mid=0;
	float  dt=4.0;
	int    *r, argindex=3,
		//  bt=20,
		principle=0,//whether compute the initial edge response 
		thin_post_proc=1,
		// three_by_three=0,
		drawing_mode=1,     //mark edge pixel black
		max_no_corners=1850,
		max_no_edges=2650,
		susan_quick=0;      
