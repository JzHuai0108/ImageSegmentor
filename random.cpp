...
Real RUniform::number(const Real a,const Real b)
{
	if ( a > b && ostr )
        {
               cout<< "RUniform::number"<<*ostr;
		cout << "Argument Error: a (" << a << ") > b (" << b
				 << ')' << endl;
	}
...
{				/* return an integer in i, i+1, ... n */
	if (i > n && ostr )
        {
                cout<<"IUniform::number"<<*ostr;
		cout<<"Argument Error: i (" << i << "> n (" << n
		      << ')' << endl;
	}
...
Real Erlang::number(const Real x,const Real s)
{
	...
	if (s > x && ostr )
	{
	cout<<"Erlang::number"<<*ostr;
		cout<<"Argument Error: s (" << s << " > x (" << x
	              << ')' << endl;
	}
...
Real Hyperx::number(const Real x,const Real s)
{
	...
	if ( s <= x && ostr )
	{
	       cout<<"Hyperx::number"<<*ostr ;
		cout<<"Argument Error: s (" << s << " not > x ("
			<< x << ')' << endl;
	}
